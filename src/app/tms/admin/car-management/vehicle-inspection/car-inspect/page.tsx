  ...
  const submitInspectionToBackend = useCallback(async (payload: InspectionPayload) => {
    ...
    try {
      ...
      if (carTypeParam !== 'organization') {  
        ...
        router.push('/tms/admin/car-management/vehicle-inspection');
        return null;
      }

      // --- >>> Redirect AFTER potential status update or skip - Choose correct results page <<< ---
      const resultPagePath = carTypeParam === 'organization'
        ? `/tms/admin/car-management/vehicle-inspection/org-result?inspectionId=${result.id}`
        : `/tms/admin/car-management/vehicle-inspection/result?inspectionId=${result.id}`;
