            ...
            {records.map((record) => (
              <tr key={record.id} className="hover:bg-slate-50 transition-colors duration-150">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <Link href={`/tms/admin/car-management/maintenance-records/${record.id}`} className="text-blue-600 hover:text-blue-800 flex items-center justify-end">
                      <FiEye className="mr-1" /> View Details
                  </Link>
                  <Link href={`/tms/admin/car-management/update-maintenance-record/${record.id}`} className="ml-4 text-green-600 hover:text-green-800 flex items-center justify-end">
    ...
    <div className="container mx-auto p-4 sm:p-6 lg:p-8 bg-slate-50 min-h-screen">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b border-slate-300 pb-4 gap-4">
          ...
          <Link href="/tms/admin/car-management/add-maintenance-record" className="w-full sm:w-auto px-4 py-2 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 flex items-center justify-center">
