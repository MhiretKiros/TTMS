...
export default function RentalRequestForm() {
  ...
  const fetchPendingRequests = async (): Promise<PendingRequest[]> => {
    try {
      ...
      if (response.data.codStatus === 200) {
        ...
        return requests
          .filter((req: any) => req.position !== 'Level 1')
          .map((req: any) => ({
            id: req.id.toString(),
            formData: {
  ...
  const handleAssign = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (eligibleCars.length === 0) {
        ...
        if (formData.driverLicense) {
          formDataToSend.append('driverLicenseFile', formData.driverLicense);
        }

        const endpoint = `${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/car/assign`;
      ...
      setAssignmentResult({
        success: false,
        message: error instanceof Error ? error.message : 'Error processing assignment'
      });
    } finally {
  ...
  const confirmAutoAssignment = async () => {
    ...
    try {
      ...
      if (request.driverLicense) {
        // If driverLicense is a URL, you might need to fetch it first
  ...
  // ... (keep all the existing JSX rendering code the same)
