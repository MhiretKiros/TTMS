...
interface AssignmentHistory {
  ...
  driverLicenseFilepath: string | null;
  driverLicenseFileType: string | null;
  ...
  useEffect(() => {
    const loadAssignment = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/assignment/${id}`);
        const data: ApiResponse = await response.json();
        
        if (data.codStatus === 200) {
          if (data.assignmentHistory) {
            setAssignment(data.assignmentHistory);
            // Load license preview if available
            if (data.assignmentHistory.driverLicenseFilename) {
              const isImage = data.assignmentHistory.driverLicenseFileType?.startsWith('image/');
              if (isImage) {
                const fileUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/licenses/file/${data.assignmentHistory.driverLicenseFilename}`;
                setLicensePreview(fileUrl);
              }
            }
            ...
            if (foundAssignment) {
              ...
              if (foundAssignment.driverLicenseFilename) {
                const isImage = foundAssignment.driverLicenseFileType?.startsWith('image/');
                if (isImage) {
                  const fileUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/licenses/file/${foundAssignment.driverLicenseFilename}`;
                  setLicensePreview(fileUrl);
                }
              }
            } else {
    ...
    return (
      ...
      >
        ...
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          onClick={() => router.push("/tms/admin/car-management/assign-car")}
...
const renderLicenseInfo = () => {
  ...
  // Use assignment ID to get the file
  const licenseUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/licenses/file/${assignment.id}`;
  
  // Determine file type more reliably
  const isImage = assignment.driverLicenseFileType?.startsWith('image/') ?? 
                 assignment.driverLicenseFilename?.match(/\.(jpg|jpeg|png|gif|webp|bmp)$/i);

  const isPDF = assignment.driverLicenseFileType?.includes('pdf') ?? 
               assignment.driverLicenseFilename?.match(/\.pdf$/i);

  return (
    <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
      <h3 className="text-lg font-semibold mb-3">Driver License Information</h3>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* License Number and Expiry Date (same as before) */}
        ...
        <div className="md:col-span-2">
          ...
          {isImage ? (
            <div className="flex flex-col sm:flex-row gap-4 items-start">
              ...
              <div className="border rounded-md p-1 bg-white max-w-xs">
                <img 
                  src={licenseUrl}
  ...
  // ... (keep all other existing functions the same: renderVehicleInfo, renderVehicleStatus, etc.)
          ...
          >
            ...
            <div className="space-y-4">
              {[
                ...
                { label: "Travel Work %", value: assignment.travelWorkPercentage },
                { label: "Short Notice %", value: assignment.shortNoticePercentage },
