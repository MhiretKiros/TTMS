...
export default function CarAssignment() {
  ...
  const handleViewAssignment = (id: number) => {
    router.push(`/tms/admin/car-management/assign-car/view-assignment/${id}`);
  };
...
// Update the renderLicenseSection function to match the working version
    ...
    >
      ...
      <div className="space-y-4">
        {[
          ...
          { 
            label: 'Expiry Date', 
            value: formatDateForDisplay(assignment.licenseExpiryDate),
            isExpired: assignment.licenseExpiryDate && new Date(assignment.licenseExpiryDate) < new Date()
          },
        ].map((item, index) => (
          <div key={index} className="flex border-b pb-3 border-gray-100">
            <span className="font-medium text-gray-600 w-48">{item.label}:</span>
            <span className={`text-gray-800 ${item.isExpired ? 'text-red-600 font-bold' : ''}`}>
              {item.value}
              {item.isExpired && (
...
// Also update the showExpiryAlert function to use the correct endpoint
const showExpiryAlert = (assignment: Assignment, daysRemaining: number, isExpired: boolean) => {
  const plateNumbers = assignment.allPlateNumbers || assignment.car?.plateNumber || assignment.rentCar?.plateNumber || 'N/A';
    ...
    if (isImage) {
      alertContent += `
        <div class="mt-3">
          ...
          <img src="${licenseUrl}" 
               class="max-w-full max-h-40 border border-gray-200 rounded mt-1" 
               onerror="this.onerror=null;this.src='/image-not-available.png';" />
        </div>
      `;
    } else {
      alertContent += `
        <div class="mt-3">
          <p class="font-semibold">License Document:</p>
          <a href="${licenseUrl}" 
             target="_blank" class="text-blue-600 hover:underline">View Document</a>
        </div>
      `;
    }
  }
