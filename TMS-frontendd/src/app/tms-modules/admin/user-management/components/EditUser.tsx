    ...
    const fetchUser = async () => {
      try {
        ...
        const response = await axios.get<UserResponse>(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/get-user/${userId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );

        if (response.data?.ourUser) {
          const { name, email, myUsername, role } = response.data.ourUser;
          setUser({
            name,
            email,
            myUsername,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put<UserResponse>(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/update/${userId}`,
        user,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end space-x-3 pt-4">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
          >
            <FiSave className="mr-2" /> {loading ? 'Updating...' : 'Update User'}
          </motion.button>
        </div>
      </form>
    </div>
