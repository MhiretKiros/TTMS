...
export default function RentalRequestForm() {
  ...
  const confirmAssignment = async (rentalType: string) => {
    ...
    try {
      ...
      // For organizational rentals, use rent-car specific endpoint and data structure
      if (rentalType === 'organizational') {
        // RentCar assignment endpoint
        await axios.post('http://localhost:8080/auth/rent-car/assign', {
          ...formData,
          totalPercentage,
          carId: proposedCar.id,
          requestDate: new Date().toISOString() // 30 days default
        });
  
        // RentCar status update
        await axios.put(
          `http://localhost:8080/auth/rent-car/status/${proposedCar.plateNumber}`,
          { status: 'Assigned' }
        );
      } else {
        // Regular car assignment
      ...
      await fetchApprovedCars(rentalType);
